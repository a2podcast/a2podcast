...
4. **Sezione Sinossi**: aggiungila in fondo (dopo le Note) come `## Sinossi` (H2). Sotto, la
   nota fissa qui sotto, poi i blocchi numerati della Fase 3.
...
10. **Accenti**: prima di mostrare il diff, correggi eventuali apostrofi usati al posto degli
   accenti (`e'`, `piu'`, `qualita'`, `perche'`, `puo'`).
11. **Non** aggiungere player, badge, "dove trovarci", newsletter (li gestisce il template).
